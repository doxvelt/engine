...
> Canonical reality is singular within a branch, but access to it is subjective.

Characters do not all receive the same world state. Each actor's turn context is assembled at a specific branch head from the perceptions, beliefs, memories, affiliations, artifacts, surfaces, and connection fabric available to that actor at that moment.

Alternative edits, regenerations, and forks are preserved as different branches. They do not coexist as canonical truth inside one branch. See [Branching and Memory](BRANCHING_AND_MEMORY.md).
...
2. Runtime fabric
   - Immutable content revisions plus branch-linked events, perceptions, beliefs, surfaces, memories, and derived projections.
   - Not edited directly by users in MVP.
   - Can preserve source spans back to dossier prose.

3. Turn context
   - Temporary prompt context constructed for one actor at one branch head.
   - Includes only what the actor can currently access or retrieve.
   - Is a pure projection and never changes simulation state merely by being read.
...
Canonical truth always exists for each branch in the engine. Unknown-to-author mysteries are not unresolved truths; they are branch-valid canonical truths hidden from the author UI or ordinary author view.
...
- Keep immutable, branch-linked belief history.
...
- Use explicit memory/belief operations when a revision, consolidation, or retraction must preserve causal intent.
- A later belief can supersede earlier beliefs by strength, recency, provenance, and branch ancestry while keeping the earlier journey intact.
...
First impressions are generated once per observer-entity encounter history, except artifacts may use looser inspection rules. First and recent impressions should carry relatively high weight during context assembly because they are dramatically useful and psychologically plausible.

Initial first impressions may be deterministic and automatic. When an accepted event gives an actor a first perception of another entity, the same committed transaction may create a `+1` observed-belief proposal derived from projected surface. Context inspection itself never creates an impression. AI-generated or player-reviewed impressions can come later.
...
Memories are actor-subjective, branch-bound interpretations derived from perceptions and prior memories. They are not just facts. They compact experience into the actor's voice, meaning, and emotional interpretation.

Memory types may include:

- episode memories proposed at closure and searchable later;
- long-term memories with stronger influence on future behavior;
- relationship, commitment, and unresolved-intention memories;
- revisions, consolidations, and retractions of earlier memory state.

Agents and dedicated memory-writer runtimes propose natural-language and structured memory operations. Doxvelt validates provenance and commits the operations; agents never write persistence directly.

The current memory state is projected at a branch head. Editing, regenerating, or forking excludes downstream memory operations from the replacement branch without deleting them from their original path.

Affiliations may hold beliefs and provenance, but subjective memories remain agent-centered initially.

## Context Assembly

An actor's turn context at one branch head may include:
...
- Current branch and episode context.
...
Source-linked beliefs and memories should carry provenance information. For example: "held by Mafia; accessed through your affiliation path." The source may affect how much the agent trusts, uses, questions, or internalizes the belief.

Context assembly is a pure projection. Perceptions, first impressions, belief changes, memory operations, and access changes require committed events.
...
Retained knowledge should be created through branch-bound memory or belief operations, often during episode closure. If an actor had access to a source-linked belief and then loses that access, later consolidation may produce a retained belief with provenance `retained_after_access_loss`.
...
Generated hidden truths must persist across runtimes and branches. They should be stored as branch-valid runtime events or hidden content revisions, not ordinary visible dossier files.
...
- Runtime profile or model intent: an abstract reference resolved by deployment policy.
...
Users create a simulation by selecting a world, scenario, entities, connections, and format. Entities may recommend a runtime profile. Provider credentials and concrete deployment endpoints remain outside portable content.
