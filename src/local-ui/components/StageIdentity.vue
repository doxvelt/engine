...
const props = defineProps<{ actorId: string; actors: StageActor[] }>();
const actor = computed(() => props.actors.find(item => item.id === props.actorId));
const name = computed(() => actor.value?.name || props.actorId);
</script>
